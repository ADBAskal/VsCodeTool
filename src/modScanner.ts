...
export interface ModCandidate {
    name: string; // The PBO Name (from CfgPatches)
    path: string; // The Source Folder Path
    hasConfig: boolean;
    needsBuild: boolean;
    status: 'ok' | 'modified' | 'pending'; // ok=synced, modified=source>pbo, pending=no pbo
    lastModified: number;
    pboPath?: string; // Target PBO Path for debugging
}
...
export class ModScanner {
    ...
    private async scanFolderRecursively(folderPath: string, candidates: ModCandidate[]) {
        const baseName = path.basename(folderPath);
        // Skip hidden/system folders
        if (baseName.startsWith('.') || baseName === 'node_modules' || baseName === 'media' || baseName === 'out') return;
        ...
        if (await this.hasConfig(folderPath)) {
            ...
            let status: 'ok' | 'modified' | 'pending' = 'pending';
    ...
    private async extractPboName(dirPath: string): Promise<string> {
